...
export default function MegaMenuServices() {
    ...
    return (
        <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-3 py-5">
            {/* LEFT IMAGE */}
            <div className="rounded-xl overflow-hidden lg:p-5 p-3 bg-gray-500">
            ...
            <div className="flex flex-col lg:w-[55%] w-full lg:gap-4 gap-2 mt-4">
                {tabs.map((t) => (
                    <button
                        ...
                        className={`lg:py-4 py-2 px-5 rounded-full border border-gray-500 text-left transition-all
              ${activeTab === t ? "bg-gray-500 text-white" : "bg-white text-black"}`}
                    >
                        {t.replace(/([A-Z])/g, " $1")}
                    </button>
                ))}
            ...
            <div className="hidden lg:block">
                <div className="mb-2 text-[20px] text-gray-500 font-semibold">Services</div>
                        ...
                        {data[activeTab].map((item) => {
                            ...
                            return (
                                <motion.div
                                    ...
                                    className={`cursor-pointer p-4 rounded-2xl flex items-start gap-3 transition-all
                    ${isHover ? "bg-white text-black shadow-lg" : "bg-white text-black"}`}
                                >
                                    <div
                                        className={`w-14 h-14 rounded-xl flex items-center justify-center transition-all
                      ${isHover ? "bg-gray-500 text-white" : "bg-gray-100 text-gray-500"}`}
                                    >
                                        <Icon size={23} />
                                    </div>
